...
export async function POST(request: NextRequest, { params }: { params: { applicationId: string } }) {
  try {
    ...
    // Regenerate feedback
    const matchScore = await calculateMatchScore(application.candidateId, application.jobId)
    const feedbackSuggestions = await generateFeedback(application.candidateId, application.jobId, matchScore)
